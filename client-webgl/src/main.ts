...
// Systemic safety net for the boot path. iOS Safari does not reliably fire
// `unhandledrejection` for top-level-await rejections, so any throw inside
// `entrypoint()` would otherwise vanish from server.log. Routing through
// `window.__report` (installed by the inline script in index.html) sends
// it to /api/log for inspection. Re-throw so devtools still surfaces it.
entrypoint().catch((err: unknown) => {
  const e = err as { name?: unknown; message?: unknown; stack?: unknown };
  const report = (window as unknown as { __report?: (p: object) => void }).__report;
  report?.({
    kind: 'entrypoint-failed',
