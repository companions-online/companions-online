...
systems/harvest.ts       Channeled gathering, auto-pathfind to adjacent, tree depletion → returns HarvestEvent[]. First-yield (adjacent-immediate / pathfinding→active transition) writes world.cooldowns at tickCost-1 ("pre-commit channel" — start tick already counts); per-yield writes tickCost. cancelHarvest preserves cd (post-yield rate residue).
