...
game-world.ts            GameWorld class — state container + runTick orchestration + player lifecycle + observer lifecycle (addObserver/removeObserver/setObserverFocus + ObserverSlot) + event emission (public emitEvent point-to-point + broadcastEvent to nearby + makeEvent + bpName). processEntityDeath + handlePlayerDeath + clearAiTargetsOn live here; entity-spawn primitives extracted to entity-spawn.ts; action dispatch moved to world-actions.ts. broadcastTick uses streamToTarget(centerX,centerY,...) helper shared between player + observer loops; broadcastEvent + setEntityMeta also iterate observers in range. Weather + tickOffset + effectiveTick getter; env section emission on keyframe crossings.
