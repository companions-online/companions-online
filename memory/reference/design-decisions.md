...
**Unified per-entity cooldown over per-state timers.** Every time-taking action used to carry its own pacing field (`MovementState.cooldownRemaining`, `HarvestState.ticksRemaining`, `CombatState.ticksRemaining`, `ConsumableState.ticksRemaining`). Two consequences: (1) `setMoveTarget` recreated `MovementState` with `cooldownRemaining: 0` on every call, and 1-tile paths destroyed the moveState before the post-step cd could persist — so re-issuing MoveTo every tick (WASD-cycle / click-spam) gave 1 step per server tick = 20 tiles/sec instead of `speed`. (2) Cross-system pacing was unenforced — chaining move→harvest→move had no shared rate-limit. Replaced with `world.cooldowns: Map<eid, ticks>` decremented at top of `runTick`, gated by every commit site, written by every committing action. Survives moveState destruction (closes the exploit). Dispatch is intentionally NOT gated — `processAction` always reaches the handler, `cancelConflictingStates` swaps state, only the *commit* waits on cd. That preserves the "click-tree-then-click-deer" UX (no rejections, smooth re-aim). Cancel semantics: `cancelConsume` and explicit `handleCancel` clear cd (in-flight channel / explicit stop, no commit). `cancelHarvest` clears cd only in the channel phase — pathfinding-phase cd is `runMovement`'s post-step residue and clearing it would let `Harvest(far) ↔ MoveTo` alternation bypass the step rate. `cancelCombat` always preserves cd: combat's first swing is free (`startAttack` is cd-silent), so a clear there would re-trigger the free swing per cancel cycle and unbounded the swing rate. The asymmetry between harvest and combat traces to whether `start*` writes cd at registration: harvest does (so cancel+restart rewrites cd to its full `tickCost - 1` — rate-neutral), combat doesn't (so cancel+restart would skip pacing entirely). Pre-commit writes use `value - 1` to preserve the original "N ticks of channeling" contract (start tick already counts as tick 1); post-commit writes use the full value. Same-tick collisions handled by `setCooldown` max-write — the longest in-flight commit wins.

**`world-actions.ts` as a flat sibling, not a folder.** Action dispatch was extracted from `game-world.ts` into one file at the same level — no `actions/` folder. Folders are for encapsulation (subsystems with internal helpers, like `mcp/` is now — `tools.ts`, `formatters.ts`, `session.ts`, `config.ts`); flat-with-prefix is for the kind of code that's tightly coupled to its sibling and imported widely (action dispatch is called by tests, MCP, and the tick loop directly). A future `world-tick.ts` pass is queued for the same reason. `game-world.ts` stays as the state container + tick orchestrator; `world-actions.ts` holds dispatch + handlers + `rejectAction` + `executeInteract` + `toggleDoor`.
...
**Tick phase order: move→resolve→channel→resolve-damage→observe** — actions → critterAI → respawns → **movement** → **pendingActions** → **harvest** → consumables → combat → broadcast → cleanup. The unified `pendingActions` resolver and harvest's pathfinding→channel transition both sit right after movement so `hasMoveTarget` reflects post-move state. The previous layout ran harvest *before* movement, which left the distance-harvest case racing: movement's `clearMoveTarget` would flip `currentAction=Idle` at phase 5 and the MCP tool would resolve before harvest (phase 3 of the *next* tick) could promote to `Harvesting`. Moving harvest after movement makes that flip happen atomically within one tick. Combat stays last among resolution phases so damage/death is the "final word" before broadcast.

**One `pendingActions` queue, not N "pending X" maps.** Six actions need walk-then-do semantics — pickup, interact, transfer, trade, dialogue_select, use_item_at. The earlier shape had one Map per action kind (`pendingPickups`, `pendingInteracts`) with parallel resolver blocks copy-pasted in the tick loop, and the four remaining actions hard-rejected with `not_adjacent`. Both branches of that split silently dropped pathfinding failures: the deferred-pickup handler ignored `setMoveTarget`'s `Err`, the immediate-reject handlers gave the LLM no path-to-target affordance. Unifying gives one resolver, one cancellation rule, and one place where every walk-to-act failure surfaces a structured rejection. `PendingAction.execute` is a closure that re-runs the action's full pre-conditions on arrival — a moving target / depleted inventory / dialogue option that vanished produces the right rejection at the right time. Mobile entity targets get re-aimed when their tile coord changes (cost bounded by target speed, not server tick rate). The same closure pattern handles unwalkable goals: `setMoveTarget('near')` + `findPath`'s adjacent-fallback routes the player to a walkable shore tile when the target is e.g. a river, so `use_item_at(woodFloor, riverX, riverY)` works without per-handler neighbor enumeration.
...
**`isPlaceable` is itself a two-mode predicate** — an earlier shape rejected any tile with an existing building outright (`current !== None → false`), which silently broke the furnish-a-house case: placing a chest / door / campfire on top of a wooden floor got rejected as `tile_blocked/wall`, and door placement (which separately *requires* a floor underneath) became unreachable through the real flow because the predicate fired first. The fix splits on `newBuilding === null`: entity placements delegate to `isWalkable(terrain, current)` — "you can put an entity wherever a player could stand" — so floors-as-platforms work for free, future walkable building types stay in lockstep, and the building-tile branch keeps the original no-stacking + terrain rules. Reusing `isWalkable` rather than open-coding the entity rule means there's exactly one place that defines "what counts as a standable surface." Three orthogonal storage layers (terrain byte / building byte / entity store) coexist on a tile; `isPlaceable` is what mediates whether a fourth element joins them.
...
**Asset folder taxonomy mirrors `Blueprint.category`.** `client-webgl/assets/` is partitioned into `creatures/`, `npcs/`, `items/{tools,weapons,armor,consumables,misc}/`, `resources/`, `placeables/` (the five blueprint categories + an items-by-blueprint-ID-block split), plus sibling `effects/` and `ui/` for non-blueprint art. `SpriteManifestEntry.filename` is the full relative path without extension (e.g. `'items/tools/axe'`); `sprite-registry.ts` resolves it against `/assets/`. Filenames are kebab-case. Reason: the manifest is the single source for asset paths, and pinning it to the blueprint taxonomy keeps "where does X live" answerable from `blueprints.ts` alone.
