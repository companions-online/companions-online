...
- **HUD action button bar (mobile-friendly tap loop)** — `client-webgl/src/ui/hud-buttons.ts` adds a three-button row at the bottom-right of the play area: `[action][inventory][settings]`. The action button label switches on `selectedMode(scene)` (`Place {name}` / `Cook {name}` / `Eat {name}` / hidden for tools / none); tapping it either fires `UseConsumable` immediately or arms `scene.armedAction: 'placement' | 'cook' | null`. Once armed, the next left-click on a tile commits the action via `UseItemAt` (or `handleCookingClick`) and **stays armed** for the next tap — sticky chaining for stacks of walls / raw fish. Auto-clears via a lazy stale-check in the mouse handler when `selectedMode(scene) !== armedAction` (stack ran out / slot rebound); explicit cancels via the action button toggle, a different quickslot select, Esc, or any modal overlay opening. Right-click + `i` + Esc desktop gestures unchanged — the bar is purely additive. Mouse-handler dispatch ladder reorganized in `controls/mouse.ts`: overlay-capture → HUD-quickbar select → HUD-button click → armed tap-to-act → right-click contextual → left-click world. Top-left action-label HUD readout (resolved action under cursor) is now always visible (was gated on a `q`-toggled debug mode that was removed). Player + chest weight caps lifted to `MAX_PLAYER_WEIGHT = 100` and `MAX_CHEST_WEIGHT = 250` (single source in `shared/src/inventory.ts`); `InventoryManager.create` argument is now required (no default 50). Coverage: `test/client-gl/hud-buttons.test.ts` (16), expanded `controls.test.ts` (HUD-quickbar click + button click + armed tap-to-act + sticky arming + stale-arm fallthrough). Sticky-arming UX rule captured in `memory/feedback/sticky-arming.md`.
