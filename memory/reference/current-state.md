...
- **Standalone HTML + dev server retired; docs split standalone vs observer** (2026-05-10) — the parallel `client-webgl/index-standalone.html` + `client-webgl/dev-standalone.ts` + `npm run dev:standalone` (esbuild serve on :3002) are gone. Single bundle now handles both deployment shapes (game-server-served + Docusaurus-embedded); standalone vs networked is a menu choice, not an HTML-file toggle, and **standalone is always available** regardless of `GAME_SERVER_HOST` (which only autofills the Join Game host field). The memory entry `memory/client-webgl/standalone-observer.md` was split into two: `memory/client-webgl/standalone.md` (the in-browser build with embedded GameWorld) and `memory/client-webgl/observer-mode.md` (the passive-viewer concept, server-side seam, today consumed by the menu backdrop). Cross-references updated in `overview.md`, `file-map.md`, `architecture.md`, `menu.md`, and `reference/architecture.md`.
