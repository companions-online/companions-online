...
The harness drives an LLM through the game as a player, via the same MCP surface a real game client uses. Two roles:
...
A third entry, `npx harness human <model>`, swaps the LLM decider for a TTY menu — useful for debugging the prompt and tool surface without spending model tokens.
...
- `cli/` — the two `bin` entries + a shared SIGINT/abort helper.
- `variants/` — one `VariantStrategy<S>` per history-management style. No CLI, no IO.
- `helpers/` — `runner.ts` (the loop), `bootstrap.ts` (wiring), and the IO + integration modules (mcp, openrouter, scratchpad, dispatcher, logger, env, paths, config).
