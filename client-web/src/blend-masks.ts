 ...
 *
 * 0.70 for rough gives the ~1/4-diamond coverage seen in the blendomatic.md
 * ASCII schematics. 0.50 is the full half-diamond (gentle gradient for sand).
 * 0.55 pulls the water shore a little tighter but still leaves a visible band.
 */
...
// ---------------------------------------------------------------------------
// Debug helper — lets Phase A verify mask shapes visually before Phase C wires
// them into real terrain rendering.
// ---------------------------------------------------------------------------

const ATLAS_PADDING = 8;
const ATLAS_LABEL_H = 14;

/**
 * Draw a 3 × 31 atlas of all generated masks, scaled 2×, with mode/mask labels.
 * Masks are white-alpha on a dark background so shape is legible.
 * Toggle on via config.DEBUG_MASK_ATLAS — see renderer.ts.
 */
export function drawMaskAtlas(
  ...
  const scale = 2;
  const cellW = TILE_W * scale + ATLAS_PADDING;
  const cellH = TILE_H * scale + ATLAS_PADDING + ATLAS_LABEL_H;

  const rowLabels = ['rough', 'smooth', 'short'];

  ctx.font = '10px monospace';
  ctx.textBaseline = 'top';

  // Column headers (mask id).
  ctx.fillStyle = '#888';
  for (let k = 0; k < MASKS_PER_MODE; k++) {
    const x = 40 + k * cellW;
  ...
  for (let m = 0; m < BLEND_MODE_COUNT; m++) {
    const rowY = 20 + m * cellH;

    // Row label.
    ctx.fillStyle = '#888';
    ctx.fillText(rowLabels[m], 4, rowY + TILE_H * scale / 2 - 5);

    for (let k = 0; k < MASKS_PER_MODE; k++) {
      const cellX = 40 + k * cellW;
      const cellY = rowY;

      // Cell background so alpha is visible.
      ctx.fillStyle = '#262626';
      ctx.fillRect(cellX, cellY, TILE_W * scale, TILE_H * scale);

      ctx.imageSmoothingEnabled = false;
      ctx.drawImage(
        masks[m][k],
        0, 0, TILE_W, TILE_H,
        cellX, cellY, TILE_W * scale, TILE_H * scale,
      );

      // Cell border.
      ctx.strokeStyle = '#333';
      ctx.strokeRect(cellX + 0.5, cellY + 0.5, TILE_W * scale - 1, TILE_H * scale - 1);
    }
  }
}
