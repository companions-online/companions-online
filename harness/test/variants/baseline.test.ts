...
describe('baseline harness', () => {
  it('accumulates full history and pings "continue" after each tool result', async () => {
    ...
    const memory = openScratchpad('session-baseline', memDir);
    ...
    expect(msgs2.map(m => m.role)).toEqual(['system', 'user', 'assistant', 'tool']);

    // Turn 3: each prior turn appended verbatim — 6 messages total.
