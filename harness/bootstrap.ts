...
import { createDispatcher, type ToolDispatcher } from './tools.js';
import { buildHarnessTools } from './harness-tools.js';
import { openMemoryFile, type MemoryFile } from './memory-file.js';
...
function loadConfig(configDir: string, name: string): ModelConfig {
  const path = join(configDir, `${name}.json`);
  const cfg = JSON.parse(readFileSync(path, 'utf8')) as ModelConfig;
  if (!cfg.model) throw new Error(`config ${path} missing "model"`);
  return cfg;
}
...
export async function bootstrapHarness(opts: BootstrapOpts): Promise<Bootstrap> {
  ...
  const configDir = opts.configDir ?? 'harness/config';
  const config = loadConfig(configDir, opts.configName);
  const { system, first } = loadPrompt(configDir);

  const sessionId = opts.sessionId ?? randomUUID();
  const log = opts.logger ?? createLogger(sessionId);
  const memory = opts.memory ?? openMemoryFile(sessionId);
  ...
  if (!decider) {
    ...
    const { model: _m, actionWindowSize: _a, ...extra } = config;
