...
import { runEval, type EvalConfig } from '../eval-runner.js';
...
import { createNoopLogger } from '../../test/helpers/noop-logger.js';
...
describe('runEval', () => {
  it('resolves AI eid, accumulates tokens, stops at max_turns, writes result file', async () => {
    ...
    const result = await runEval({
      ...
      memory: openMemoryFile('eval-test-1', memDir),
      resultsDir,
    });
  ...
  it('records a checkpoint hit when the AI harvests a tree', async () => {
    ...
    const result = await runEval({
      ...
      logger: createNoopLogger(),
      memory: openMemoryFile('eval-test-2', memDir),
      resultsDir,
    });
  ...
  it('stops on max_tokens when the budget is exhausted', async () => {
    ...
    const result = await runEval({
      ...
      worldFactory: () => createTestWorld(),
      logger: createNoopLogger(),
      memory: openMemoryFile('eval-test-3', memDir),
      resultsDir,
    });
