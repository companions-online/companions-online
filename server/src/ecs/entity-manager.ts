...
export class EntityManager {
  ...
  readonly blueprintId   = new ComponentStore<BlueprintIdData>(ComponentBit.BlueprintId, this.dirty);
  ...
  getFullState(id: number): { components: EntityComponents; speed?: number } {
    ...
    const bp = this.blueprintId.get(id);
    if (bp) components.blueprintId = bp;
  ...
  getDeltaComponents(id: number, bitmask: number): EntityComponents {
    ...
    if (bitmask & (1 << ComponentBit.BlueprintId)) {
      const v = this.blueprintId.get(id);
      if (v) components.blueprintId = v;
    }
